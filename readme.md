...
Importér 'postman*collection.json' *(som ligger i `mcd-docs/postman` i dette
projekt)
...
## Simple workflow for assets and local dev

1. Sync assets from `mcd-docs` into the public folder (run after pulls):
...
- Public site assets live in `public/`.
- Raw docs and downloadable zips remain in `mcd-docs/` and are served at `/mcd-docs`.
- Use `npm run sync:assets` whenever new images are added to `mcd-docs`.

### Where the DGS image sources live now

The canonical DGS image sources (logo, header, dishes, employees) have
been moved into the frontend source so Vite will bundle them automatically
on build. You can find them here:

- `sites/www/src/assets/dgs_materialer`

Notes on workflow:

- During development the app imports these files from `src/assets` so you
  will see changes after a rebuild (`npm run build:web`) or when running
  the dev server.
- The `public/dgs_materialer` directory has been removed to avoid duplicate
  assets. If you need to publish raw documents or zips, they remain available
  under `mcd-docs/` and are served at `/mcd-docs`.

If you prefer keeping a copy of the DGS assets in `public/` for direct
access (e.g., for non-bundled references), run:
